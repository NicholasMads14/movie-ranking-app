      ...
      <div className="flex flex-col items-center justify-center min-h-[85vh] px-4">
        <div className="w-full max-w-sm flex flex-col gap-6">
          <div className="text-center">
            <p className="text-2xl font-bold text-gray-900 dark:text-white">All caught up.</p>
          ...
          <input
            ...
            className="w-full bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 text-gray-900 dark:text-white rounded-xl px-4 py-3 text-sm outline-none focus:border-gray-400 dark:focus:border-gray-600 transition-colors placeholder-gray-400 dark:placeholder-gray-600"
          />
          <p className="text-gray-500 dark:text-gray-600 text-s text-center">Head over to <span className="text-gray-700 dark:text-gray-400">Reorder</span> to fine-tune your rankings.</p>
          {reRankSearch && (
              ...
              {filteredMovies.map(m => {
                ...
                return (
                  <button
                    ...
                    className="flex items-center justify-between bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 hover:border-gray-400 dark:hover:border-gray-600 rounded-xl px-4 py-3 transition-colors text-left"
                  >
                    <span className="text-gray-900 dark:text-white text-sm">{m.title}</span>
                    {tierInfo && (
                      <span
        ...
        {!reRankMovie && (
          <div className="flex flex-col gap-3">
            <div className="flex justify-between items-center">
              <span className="text-xs text-gray-400 dark:text-gray-600 uppercase tracking-widest">Your ranking</span>
              <span className="text-xs text-gray-400 dark:text-gray-600">{progress} / {movies.length}</span>
            </div>
            <div className="w-full bg-gray-200 dark:bg-gray-900 rounded-full h-px">
              <div
                className="bg-gray-500 dark:bg-gray-400 h-px rounded-full transition-all duration-500"
                style={{ width: `${pct}%` }}
              />
            </div>
          </div>
        ...
        {reRankMovie && (
          <div className="flex items-center gap-3">
            <button
              ...
              className="text-gray-400 dark:text-gray-600 hover:text-gray-700 dark:hover:text-white transition-colors text-sm"
            >
              ← back
            </button>
            <span className="text-xs text-gray-400 dark:text-gray-600 uppercase tracking-widest">Re-ranking</span>
          </div>
        )}

        <div className={`text-center transition-opacity duration-150 ${animating ? 'opacity-0' : 'opacity-100'}`}>
          <h2 className="text-4xl font-bold text-gray-900 dark:text-white tracking-tight leading-tight">
            {current.title}
          </h2>
        ...
        <div className="flex flex-col gap-2">
          {TIERS.map(tier => (
            <button
              ...
              disabled={saving}
              className="group relative w-full text-left px-5 py-4 rounded-2xl bg-gray-100 dark:bg-gray-900 border border-gray-200 dark:border-gray-800 disabled:opacity-40 transition-all duration-150 overflow-hidden"
            ...
            >
              ...
              <div className="flex items-center gap-3 relative px-4">
                <div className="w-2 h-2 rounded-full flex-shrink-0 border border-gray-300 dark:border-gray-700" style={{ background: tier.accent }} />
                <span className="text-gray-900 dark:text-white font-medium text-sm">{tier.label}</span>
              </div>
            </button>
          ))}
        </div>
